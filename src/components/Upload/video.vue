...
export default {
  ...
  created() {
    ...
    if (this.imgList) {
      this.fileList = [{status: "success",name: this.imgName ? this.imgName+'.video' : 'thisImg.video',size: 63624,percentage: 100,uid: 1593691580290}]
    }
  },
  methods: {
    ...
    beforeAvatarUpload(file) {   //视频上传之前的方法
      ...
      if (['video/mp4', 'video/ogg', 'video/flv', 'video/avi', 'video/wmv', 'video/rmvb', 'video/mov'].indexOf(file.type) == -1) {
         this.$message.error("上传头像视频只能是 mp4、ogg、flv、avi、wmv、rmvb、mov 格式!");
         return false;
       }
       if (!isLt2M) {
         this.$message.error("上传头像视频大小不能超过 1G!");
         return false;
       }


      let keyName = Date.parse(new Date()).toString()+Number.parseInt(Math.random()*1000, 10)+'.video'
