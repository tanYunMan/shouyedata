    ...
    <div class="recevie-wrap">
      <el-table border row-key="id" ref="multipleTable" :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" :header-cell-style="{background:'#f5f5f5',color:'#606266','text-align':'center'}" v-loading="loading" size="mini" :cell-style="{'text-align':'center'}">
        <!--  <el-table-column :show-overflow-tooltip="true" type="selection" > </el-table-column> -->
         ...
         <el-table-column :show-overflow-tooltip="true" prop="userName" label="客户登录名"></el-table-column>
         ...
         <el-table-column :show-overflow-tooltip="true" prop="label10" label="密码重置"f width="400">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="ResetPassword(scope.row,1)">登录密码</el-button>
            <el-button size="mini" @click="ResetPassword(scope.row,2)">交易密码</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="ResetInfo.title" :visible.sync="dialog1" class="dialog1" width="40%">
      <ul class="reset_ul">
        <li>
          ...
          <span class="val">{{ResetInfo.localUserID || ''}}</span>
        </li>
        <li>
        ...
        <li>
          ...
          <span class="val">{{ResetInfo.userLevelName || ''}}</span>
        </li>
        <li>
          <span class="label">客户地址：</span>
          <span class="val">{{ResetInfo.address || ''}}</span>
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      getLists(this.queryParams,'/user/user/pageUser').then(res => {
        if (res.code===1001){
    ...
    ResetPassword (data,type) {
      if (type === 1) {
        this.ResetInfo = {
          ...
          address: data.userArea + data.address,
          userID: data.userID,
          type: type
        }
      } else {
