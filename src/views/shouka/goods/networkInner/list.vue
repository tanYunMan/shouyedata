  ...
  <div>
    <div class="search-container">
      <el-form :inline="true" :model="searchForm" ref="searchForm">
        ...
        <el-form-item>
          <el-input v-model="searchForm.goodsName" placeholder="商品名称" style="width:250px;height:30px"></el-input>
        </el-form-item>
        <el-form-item>
          <el-input v-model="searchForm.joinCode" placeholder="对接码" style="width:250px;height:30px"></el-input>
        </el-form-item>
    ...
    >
     <el-table-column :show-overflow-tooltip="true" type="selection" align="center"></el-table-column>
      <el-table-column :show-overflow-tooltip="true" prop="goodsName" label="商品名称" align="center" min-width="35%"></el-table-column>
      <!-- <el-table-column prop="systemNoticeTitle" label="排序"></el-table-column> -->
      <el-table-column :show-overflow-tooltip="true" prop="price" label="价格" align="center" min-width="7%"></el-table-column>
      
      <el-table-column :show-overflow-tooltip="true" prop="cardNum" label="卡密数量" align="center" min-width="8%"></el-table-column>
       <el-table-column :show-overflow-tooltip="true" prop="updateTime" label="添加时间" align="center" min-width="15%"></el-table-column>
        <el-table-column :show-overflow-tooltip="true" label="商家" align="center" min-width="10%">
          <template slot-scope="scope">
         
          <el-button type="text" @click="showBusinessInfo(scope.row)">查看</el-button>
          </template>
        </el-table-column>
        <!-- <el-table-column :show-overflow-tooltip="true" label="是否对接" align="center" min-width="15%">
          <template slot-scope="scope">
            <el-button type="warning" plain size="mini" style="margin-left:0" v-show="scope.row.connected == false">未对接</el-button>
            <el-button type="primary" plain size="mini" style="margin-left:0" v-show="scope.row.connected == true">已对接</el-button>
          
          </template>
        </el-table-column> -->
      <el-table-column :show-overflow-tooltip="true" label="操作" align="center" fixed="right" min-width="10%">
        <template slot-scope="scope">
          <el-button type="text" v-if="scope.row.connected == true">已对接</el-button>
          <el-button type="primary" @click="goToDocking(scope.row)" v-else>对接</el-button>
          <!-- <el-button type="text">查看</el-button> -->
        </template>
      </el-table-column>
    </el-table>
    <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
                   :page-sizes="[10, 20, 50]" :current-page="searchForm.pageNum" :page-size="searchForm.pageSize" layout="total, sizes, prev, pager, next, jumper"
                   :total="total"></el-pagination>

    

    <!-- 查看site商家信息 -->
      <el-dialog title="商家信息" :visible.sync="businessInfoSiteDialog" width="40%" class="dialog1">
      ...
      <!-- 查看fakaset商家信息 -->
      <el-dialog title="商家信息" :visible.sync="businessInfoFakaSetDialog" width="40%" class="dialog1">
        <ul class="businessInfoUl">
          
          <li>
            <span class="label">店铺名称：</span>
            <span class="val" v-if="businessInfo">{{businessInfo.fakaName}}</span>
          </li>
            <li>
            <span class="label">店铺QQ：</span>
            <span class="val" v-if="businessInfo">{{businessInfo.fakaQq}}</span>
          </li>
            <li>
            <span class="label">店铺WX：</span>
            <span class="val" v-if="businessInfo">{{businessInfo.fakaWx}}</span>
          </li>
          <li>
            <span class="label">店铺电话：</span>
            <span class="val" v-if="businessInfo">{{businessInfo.mobilePhone}}</span>
          </li>
          <li>
            <span class="label">店铺链接：</span>
              <a style="color:#409EFF" v-if="businessInfo" target="_blank" type="primary" :href="businessInfo.domain">{{businessInfo.domain}}</a>
          </li>
          <li>
            <span class="label">店铺长链接：</span>
              <a style="color:#409EFF" v-if="businessInfo" target="_blank" type="primary" :href="businessInfo.longLink">{{businessInfo.longLink}}</a>
          </li>
          <li>
            <span class="label">店铺公告：</span>
            <span class="val" v-if="businessInfo">{{businessInfo.fakaNotice}}</span>
          </li>
        </ul>
      </el-dialog>
...
export default {
  data() {
    return {
      businessInfoSiteDialog:false,
      businessInfoFakaSetDialog:false,
      businessInfo:{},
      dockingDialog:false,
      ruleForm:{},
      multipleSelection: [],
  ...
  methods: {
    getList(){
      this.loading = true
      request({
          url:'/goods/goods/getFakaConnectGoods',
          method:'post',
    ...
    showBusinessInfo(row){
        
        if(row.fakaSet != null && row.fakaSet.longLink){
            this.businessInfo = row.fakaSet
            this.businessInfoFakaSetDialog= true
        }else{
            this.businessInfo = row.connectSite
            this.businessInfoSiteDialog= true
        }
    },
    ...
    localCatalog() {
      ...
      getDatas({},'/goods/catalogFaka/list').then(res => {
        if (res.code===1001){
          this.recombinationData = res.body
        }
      }).catch(err => {
    ...
    goToDocking(data){
      this.$router.push({
        path:'/goodsManager/netUpdate',
        query:{
          goodsSourceID:data.goodsID,
          goodsState:data.goodsState,
  ...
  created(){
    this.searchForm.pageNum =  parseInt(this.$route.params.currentPage) || 1
