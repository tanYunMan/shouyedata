<template>
  <div class="container container-good">
    ...
    <el-table border row-key="goodsID" ref="multipleTable" :data="tableData3" style="width: 100%"
      ...
      <el-table-column :show-overflow-tooltip="true" prop="label4" label="状态" align="center" width="60">
        <template slot-scope="scope">
          <el-button size="mini" type="text" @click="updateStates(scope.row)">{{scope.row.goodsState | filterState}}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column :show-overflow-tooltip="true" prop="supplierUser" label="供货商" align="center" width="160">
        <template slot-scope="{row}">
          {{row.supplierUser.userName}}({{row.supplierUser.localUserID}})
        </template>
      </el-table-column>
      ...
      <el-table-column :show-overflow-tooltip="true" prop="label9" label="库存操作" align="center" width="200">
        <template slot-scope="scope">
    ...
    <el-dialog title="批量修改商品名称颜色" :visible.sync="dialogColor" width="60%" class="dialog8">
      <el-form ref="formColor" :model="formColor" label-width="180px">
        <el-form-item label="选择商品名称颜色：" prop="color"
          ...
          <colorPicker v-model="formColor.color"></colorPicker>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
