<template>
  <div class="container menu2-1-container">
    ...
    <div class="filter-wrap-text" v-show="type == 1">
      结算周期D+1：星期一、星期二、星期三、星期四、星期五、星期六、星期日，0点~23点59分59秒间提现的资金将于次日23点59分59秒前到账。个人支付宝单笔提现最高50000元，企业支付宝单笔提现最高100000元
    </div>
    <div class="filter-wrap-text" v-show="type == 2">
      修改提现方式后,联系上级实时审核,效率更快。
    </div>
    ...
    <div class="recevie-wrap" v-show="type == 2">
      <el-form ref="form" :model="form" :rules="rules" label-width="140px" class="demo-ruleForm">
        ...
        <el-form-item label="上传图片：">
          <Upload v-if="imgLoading" :img-list="form.payImg" @listenTochildEvent="showMessageFromChild" />
        </el-form-item>
  ...
  methods: {
    ...
    submit(form){
      this.$refs[form].validate(async valid => {
        ...
        if (!this.form.cashAccount || !this.form.payImg) {
          this.$message({
            showClose: true,
            message: '请输入提现账户或上传图片',
            type: 'error',
          });
          return null
        }
