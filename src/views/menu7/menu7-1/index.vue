  ...
  <div class="container menu7-5-container">
    <el-form :inline="true" class="filter-wrap">
      ...
      <el-form-item label="用户级别">
        <el-select size="small" v-model="serchForm.levelID" placeholder="用户级别" clearable filterable>
          <el-option
    ...
    <el-table
      ...
      size="mini"
    >
      <el-table-column align="center" label="站点ID" prop="siteID" width="60px" />
      ...
      <el-table-column align="center" label="站点域名" prop="domainPre" width="180px">
        <template slot-scope="scope">
          <el-button style="padding:0;" type="text">
            <a
              target="_blank"
              :href="scope.row.systemDomain ? 'http://' + scope.row.domainPre +'.'+ scope.row.systemDomain.domainAddr : '' "
            >{{ scope.row.systemDomain ? scope.row.domainPre +'.'+ scope.row.systemDomain.domainAddr : '' }}</a>
          </el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" label="到期时间" prop="endDate" width="150px">
        <template slot-scope="scope">
          <template>
            {{ scope.row.endDate }}
            <el-button
            ...
            <div v-if="timeShow && scope.row.siteID == rowIDTime" style="display:flex;">
              ...
              <el-button
                ...
                size="mini"
                type="primary"
                @click="addTime(scope.row.siteID)"
              >续费</el-button>
            </div>
          </template>
        </template>
      ...
      <el-table-column align="center" label="站点版本" prop="location" width="140px">
       ...
       <el-table-column align="center" label="用户级别" prop="level.levelName" width="100px" />
       <el-table-column align="center" label="账号" prop="adminUser.login" width="100px" />
      <el-table-column align="center" label="联系人" prop="contacts" width="100px" />
      <el-table-column align="center" label="电话" prop="phone" width="100px" />
      <el-table-column align="center" label="证件号码" prop="iDNumber" width="150px" />
      ...
      <!-- <el-table-column align="center" label="克隆额度" prop="batchLimit" width="180px">
        <template slot-scope="scope">
          <template>
            ...
            <el-button
              v-if="(!show && scope.row.siteID == rowIDLines) || (scope.row.siteID != rowIDLines)"
              style="margin-left:20px;padding:0;"
              type="text"
            >
            ...
            <div v-if="show && scope.row.siteID == rowIDLines" style="display:flex;">
              ...
              <el-button
                ...
                size="mini"
                type="primary"
                @click="addlines(scope.row.siteID)"
              >增加</el-button>
            </div>
          </template>
        </template>
      ...
      <el-table-column align="center" label="操作" width="500px" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" @click.stop="syncProduct(scope.row)" :disabled="isDisabled">同步商品</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="pagination-box">
      ...
      <el-dialog title="修改站点" :visible.sync="updateVisible">
      <el-form ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="用户级别" label-width="120px" prop="levelID">
          <el-select 
             size="small" style="width:250px" v-model="form.levelID" placeholder="用户级别">
            <el-option
              :label="item.levelName"
              :value="item.levelID"
              v-for="item in userLevelList"
    ...
    <el-dialog :title="form.siteID ? '修改站点' : '新增站点' " :visible.sync="Visible">
      <el-form ref="form" :model="form" :rules="rules">
        ...
        <el-form-item
          label="站点域名"
          label-width="120px"
          :prop="form.siteID ? '' : 'domainID'"
          :hidden="form.siteID"
        >
          <el-input
            v-model="form.domainPre"
            size="small"
            style="width:122px"
            :disabled="form.siteID ? true :false"
          />
          ...
          <el-button
            v-if="!checking&&!form.siteID"
            size="small"
            type="primary"
            @click="checkingDomainPre"
          >验证域名</el-button>
          <el-button
            v-if="checking === 1001&&!form.siteID"
            size="small"
            type="success"
            icon="el-icon-check"
            circle
          />
          <el-button
            v-if="checking&&checking != 1001&&!form.siteID"
            size="small"
            type="danger"
            icon="el-icon-close"
            circle
        ...
        <el-form-item label="用户级别" label-width="120px" prop="levelID" 
          :hidden="form.siteID">
          <el-select 
            :disabled="form.siteID ? true :false" size="small" style="width:250px" v-model="form.levelID" placeholder="用户级别">
            <el-option
              :label="item.levelName"
              :value="item.levelID"
              v-for="item in userLevelList"
        ...
        <el-form-item>
          <el-button
            ...
            type="primary"
            @click="save('form')"
          >{{ form.siteID ? "立即保存" : "立即创建" }}</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getList(page, size) {
      ...
      getLists(form, "/platform/site/subPage").then((data) => {
    ...
    syncProduct(row){
      ...
      request({
        url: '/goods/goods/subClone',
        method: 'post',
        data:{subSiteID : row.siteID},
